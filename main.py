...
def start(message):
    ...
    bot.send_message(message.chat.id, "🚀 *Vesk-OSINT İstihbarat toplamaya hoş geldiniz.*", reply_markup=remove_markup)
...
def link_analiz_islem(message):
    try:
        r = requests.get(message.text, timeout=5)
...
def ip_analiz_islem(message):
    try:
        r = requests.get(f"http://ip-api.com/json/{message.text}", timeout=5).json()
